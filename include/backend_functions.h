...
uint8_t setCANFDBitrate(uint8_t busChannel, uint32_t mainBitrate, uint32_t dataBitrate, bool bitrateSwitch);
...
uint8_t add_to_CAN_TX_Queue(uint8_t enum_busChannel, bool EXT_ID, uint32_t ID, uint8_t DLC, uint8_t txData[8]);
