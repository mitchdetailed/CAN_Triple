...
/**
 * \brief leading zeros implementation
 * \param x : Value to look at
 * \return leading zeros count..
 */
...
/**
 * \brief trailing zeros implementation
 * \param x : Value to look at
 * \return trailing zeros count..
 */
...
/**
 * \brief Truncating of Numbers to decimal places
 * \param value Value to look at
 * \param decimal_places decimal place to truncate to (negative values to the left of the decimal point)
 * \return truncated value
 */
float truncate_to_decimal_places(float value, int8_t decimal_places)
{
    // Handle decimal_places == 0 as a special case
    if (decimal_places == 0)
    {
        return trunc(value);
    }

    // Scale factor based on the decimal places
    float scale = powf(10.0f, decimal_places);

    if (decimal_places > 0)
    {
        // Truncate to the specified number of decimal places
        return truncf(value * scale) / scale;
    }
    else
    {
        // Truncate to the specified number of digits left of the decimal point
        return truncf(value / scale) * scale;
    }
}
 ...
 * \return Float Value.
 */
float process_ieee754(uint32_t value, uint32_t bitmask, float factor, float offset, int8_t decimal_places)
{
    ...
    // Step 4: Truncate the result to the specified decimal places
    return truncate_to_decimal_places(processed_value, decimal_places);
}
 ...
 * \return Float Value.
 */
float process_float_value(uint32_t data, uint32_t Bitmask, bool is_signed, float factor, float offset, int8_t decimal_places)
{
    // Step 1: Count trailing zeros of the Bitmask
    int trailing_zeros = count_trailing_zeros(Bitmask);

    // Step 2: Right shift data and Bitmask by the trailing zero count
    data = data >> trailing_zeros;
    Bitmask = Bitmask >> trailing_zeros;

    // Step 3: Extract only the relevant bits from the data
    uint32_t extracted_data = data & Bitmask;

    float result;
    if (is_signed)
    {
        // Determine the number of bits in the Bitmask
        int bit_count = 32 - count_leading_zeros(Bitmask);

        // Check if the MSB (sign bit) is set
        if (extracted_data & (1 << (bit_count - 1)))
        {
            // Sign-extend the value for negative numbers
            extracted_data |= ~((1 << bit_count) - 1);
        }

        result = (float)((int32_t)extracted_data) * factor + offset;
    }
    else
    {
        result = (float)extracted_data * factor + offset;
    }

    // Step 4: Truncate the result to the specified decimal places
    return truncate_to_decimal_places(result, decimal_places);
}
