...
// Haltech IO Box Variables
	...
	uint32_t haltech_IO_Box_B_DPI1_Hz = 0; // min 0 Hz, max 20000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	uint32_t haltech_IO_Box_B_DPI2_Hz = 0; // min 0 Hz, max 20000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	uint32_t haltech_IO_Box_B_DPI3_Hz = 0; // min 0 Hz, max 20000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	uint32_t haltech_IO_Box_B_DPI4_Hz = 0; // min 0 Hz, max 20000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	...
	float haltech_IO_Box_B_DPO1_Period = 0.0f; // min 0.00ms, max 50.00ms, resolution 0.01ms //
	...
	uint32_t haltech_IO_Box_B_DPO1_Hz = 0; // min 0 Hz, max 1000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	uint32_t haltech_IO_Box_B_DPO2_Hz = 0; // min 0 Hz, max 1000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	uint32_t haltech_IO_Box_B_DPO3_Hz = 0; // min 0 Hz, max 1000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
	uint32_t haltech_IO_Box_B_DPO4_Hz = 0; // min 0 Hz, max 1000 Hz, resolution Variable based on Pulse Width, min 1 Hz after 300 hz, resolution will start reducing.. //
		...
		if(HALTECH_CAN == CAN_1){
			if (HALTECH_IO_BOX_B == 1){
				if(Message.arbitration_id == 0x2D1){
					haltech_IO_Box_B_DPO1_Duty = (float)((Message.data[0]<<2) + (Message.data[1]>>6))/10;
					...
					haltech_IO_Box_B_DPO1_Hz = period_10uS_to_frequency_Hz(((Message.data[1]& 0x01)<<9)+(Message.data[2]<<8)+Message.data[3]);
					haltech_IO_Box_B_DPO1_Period = (float)((((Message.data[1]& 0x01)<<9)+(Message.data[2]<<8)+Message.data[3])/10);
					haltech_IO_Box_B_DPO2_Duty = (float)((Message.data[4]<<2) + (Message.data[5]>>6))/10;
					...
					haltech_IO_Box_B_DPO2_Hz = period_10uS_to_frequency_Hz(((Message.data[5]& 0x01)<<9)+(Message.data[6]<<8)+Message.data[7]);
					haltech_IO_Box_B_DPO2_Period = (float)((((Message.data[5]& 0x01)<<9)+(Message.data[6]<<8)+Message.data[7])/10);
					
					// functions for testing DPO1
					//printf("DPO1 Period = %02.3f ms\t\t DPO1 Freq = %5i Hz\t\t DPO1 Duty = %03.1f\r\n",haltech_IO_Box_B_DPO1_Period,haltech_IO_Box_B_DPO1_Hz,haltech_IO_Box_B_DPO1_Duty);
					//printf("Byte 0 : %02x\tByte 1 : %02x\tByte 2 %02x\tByte 3 %02x\r\n", Message.data[0],Message.data[1], Message.data[2], Message.data[3]);
				}
				else if(Message.arbitration_id == 0x2D3){
					haltech_IO_Box_B_DPO3_Duty = (float)((Message.data[0]<<2) + (Message.data[1]>>6))/10;
					...
					haltech_IO_Box_B_DPO3_Hz = period_10uS_to_frequency_Hz(((Message.data[1]& 0x01)<<9)+(Message.data[2]<<8)+Message.data[3]);
					haltech_IO_Box_B_DPO3_Period = (float)((((Message.data[1]& 0x01)<<9)+(Message.data[2]<<8)+Message.data[3])/10);
					haltech_IO_Box_B_DPO4_Duty = (float)((Message.data[4]<<2) + (Message.data[5]>>6))/10;
					...
					haltech_IO_Box_B_DPO4_Hz = period_10uS_to_frequency_Hz(((Message.data[5]& 0x01)<<9)+(Message.data[6]<<8)+Message.data[7]);
					haltech_IO_Box_B_DPO4_Period = (float)((((Message.data[5]& 0x01)<<9)+(Message.data[6]<<8)+Message.data[7])/10);
				}
			}
		}
	}
...
void events_50Hz(){
	...
	if(HALTECH_IO_BOX_B == 1){
		...
		uint16_t ioBB_DPI1_Period = (uint32_t) frequency_Hz_to_period_10uS(haltech_IO_Box_B_DPI1_Hz); // Can be edited to take in a general 10uS resolution value instead of frequency conversion function
		uint16_t ioBB_DPI2_DC = (uint16_t)(haltech_IO_Box_B_DPI2_Duty * 10);
		uint16_t ioBB_DPI2_Period = (uint32_t) frequency_Hz_to_period_10uS(haltech_IO_Box_B_DPI2_Hz); // Can be edited to take in a general 10uS resolution value instead of frequency conversion function
		...
		send_message(HALTECH_CAN,false,0x2C3,8,msgIOBB_2);

		uint16_t ioBB_DPI3_DC = (uint16_t)(haltech_IO_Box_B_DPI3_Duty * 10);
		uint16_t ioBB_DPI3_Period = (uint32_t) frequency_Hz_to_period_10uS(haltech_IO_Box_B_DPI3_Hz); // Can be edited to take in a general 10uS resolution value instead of frequency conversion function
		uint16_t ioBB_DPI4_DC = (uint16_t)(haltech_IO_Box_B_DPI4_Duty * 10);
		uint16_t ioBB_DPI4_Period = (uint32_t) frequency_Hz_to_period_10uS(haltech_IO_Box_B_DPI4_Hz); // Can be edited to take in a general 10uS resolution value instead of frequency conversion function
		...
		send_message(HALTECH_CAN,false,0x2C5,8,msgIOBB_3);
		...
		send_message(HALTECH_CAN,false,0x2C7,8,msgIOBB_4);
	}
...
void events_10Hz(){
	...
	haltech_IO_Box_B_DPI1_Hz +=1;
	haltech_IO_Box_B_DPI2_Hz +=1;
	haltech_IO_Box_B_DPI3_Hz +=1;
	haltech_IO_Box_B_DPI4_Hz +=1;
	haltech_IO_Box_B_DPI1_Duty += 0.1;
	haltech_IO_Box_B_DPI2_Duty += 0.1;
	haltech_IO_Box_B_DPI3_Duty += 0.1;
	haltech_IO_Box_B_DPI4_Duty += 0.1;
	if (haltech_IO_Box_B_DPI1_Hz > 300){
		haltech_IO_Box_B_DPI1_Hz = 0;
	}
	if (haltech_IO_Box_B_DPI2_Hz > 300){
		haltech_IO_Box_B_DPI2_Hz = 0;
	}
	if (haltech_IO_Box_B_DPI3_Hz > 300){
		haltech_IO_Box_B_DPI3_Hz = 0;
	}
	if (haltech_IO_Box_B_DPI4_Hz > 300){
		haltech_IO_Box_B_DPI4_Hz = 0;
	}
	if (haltech_IO_Box_B_DPI1_Duty > 100){
		haltech_IO_Box_B_DPI1_Duty = 0;
	}
	if (haltech_IO_Box_B_DPI2_Duty > 100){
		haltech_IO_Box_B_DPI2_Duty = 0;
	}
	if (haltech_IO_Box_B_DPI3_Duty > 100){
		haltech_IO_Box_B_DPI3_Duty = 0;
	}
	if (haltech_IO_Box_B_DPI4_Duty > 100){
		haltech_IO_Box_B_DPI4_Duty = 0;
	}
